...
export function LessonBlock({ title, icon, tone = "default", children }: { title: string; icon?: string; tone?: LessonBlockTone; children: React.ReactNode }) {
  const toneClass = tone === "highlight" ? styles.lessonBlockHighlight : tone === "scripture" ? styles.lessonBlockScripture : "";
  return (
    <section className={`${styles.lessonBlock} ${toneClass}`}>
      <span className={styles.lessonBlockIcon} aria-hidden="true">{icon ?? "◇"}</span>
      <div className={styles.lessonBlockContent}><h2>{title}</h2>{children}</div>
    </section>
  );
}
    ...
    <JourneyFrame>
      <main className={styles.stepOverviewPage}>
        ...
        <div className={styles.overviewPanel}>
          <aside className={styles.yourJourney}><h2>Your Journey</h2>{journeySteps.map((item) => <Link key={item.id} href={item.href} className={item.id === step.id ? styles.currentJourneyItem : ""}><span className={styles.journeyNumber}>{item.number}</span><span>{item.title}</span><span className={styles.journeyState} aria-hidden="true">{item.id === step.id ? "●" : "○"}</span></Link>)}<div className={styles.sidebarSupport}><span className={styles.supportIcon} aria-hidden="true">♡</span><strong>You Are Not Alone</strong><p>Whatever you’re facing, we’re here to walk with you and point you to hope.</p><a href="https://988lifeline.org/" rel="noreferrer">Need Help Right Now →</a></div></aside>
...
export function SeriesOverview({ stepNumber, parentId, title, intro, sections, lessons }: { stepNumber: number; parentId: string; title: string; intro: string; sections: Card[]; lessons: JourneyLesson[] }) {
  ...
  return <JourneyFrame><main><StepBanner currentStep={stepNumber} /><div className={styles.lessonLayout}><LessonSidebar stepNumber={stepNumber} lessons={lessons} currentId={parentId} /><article className={`${styles.lessonContent} ${styles.seriesPageContent}`}><Link href={`/journey/step-${stepNumber}`} className={styles.backLink}>← Back to Step Overview</Link><header className={styles.lessonHeader}><div><p className={styles.eyebrow}>Step {stepNumber} <span>›</span> {parentId}</p><h1>{parentId} &nbsp; {title}</h1><p className={styles.lead}>{intro}</p></div><button className={styles.listenButton} type="button" disabled>◖ Listen to the Series Overview</button></header><section className={styles.seriesProgress} aria-label={`${title} progress`}><div><span className={styles.seriesProgressIcon} aria-hidden="true">▤</span><div><strong>Your Progress in This Series</strong><p>0 of {sections.length} studies completed</p></div></div><b>0%</b><div className={styles.seriesProgressTrack}><span style={{ width: "0%" }} /></div></section><div className={styles.seriesIntroGrid}><div className={styles.seriesMain}><div className={styles.seriesSectionHeading}><span aria-hidden="true">▤</span><div><h2>The {sections.length} Studies in This Series</h2><p>Each study can be completed on its own. The manuscript determines the study order and content.</p></div></div><div className={styles.cardGrid}>{sections.map((section, index) => <article className={styles.card} key={section.id}><div className={styles.cardImageSlot} aria-hidden="true"><span>Approved study image</span></div><span className={styles.lessonBadge}>{index + 1}</span><div className={styles.cardBody}><small>{section.id}</small><h2>{section.title}</h2>{section.summary && <p>{section.summary}</p>}<div className={styles.cardStatus}><span aria-hidden="true">○</span> Not Started</div><Link href={section.href}>Begin Study →</Link></div></article>)}</div><aside className={styles.seriesCompletion}><span aria-hidden="true">☆</span><div><strong>Series Completion</strong><p>When every study is intentionally marked complete, the parent lesson can be recorded as complete. Progress persistence is implemented in Sequence #8.</p></div></aside></div><aside className={styles.seriesRail}><section className={styles.seriesRailCard}><p className={styles.eyebrow}>About This Series</p><p>The authoritative manuscript determines the content and order inside each study. This overview keeps the parent lesson together while making each substantial study easier to read and return to.</p></section><section className={styles.seriesRailCard}><p className={styles.eyebrow}>Series Resources</p><p>Contextual resources can be connected here during content implementation without changing the page structure.</p></section><section className={`${styles.seriesRailCard} ${styles.seriesPrayerCard}`}><p className={styles.eyebrow}>Need Prayer?</p><p>We are here for you. Share only what you are comfortable sharing.</p><Link href="/prayer">Request Prayer</Link></section></aside></div><ContextNav previous={previous ? { href: previous.href, label: "Previous Lesson", detail: previous.title } : undefined} center={{ href: `/journey/step-${stepNumber}`, label: "Back to Step Overview" }} next={next ? { href: next.href, label: "Next Lesson", detail: next.title } : undefined} /></article></div></main></JourneyFrame>;
}
...
export function SeriesStudy({ stepNumber, parentId, sectionId, title, parentTitle, lessons, previousSection, nextSection, children }: { stepNumber: number; parentId: string; sectionId: string; title: string; parentTitle: string; lessons: JourneyLesson[]; previousSection?: NavItem; nextSection?: NavItem; children: React.ReactNode }) {
  ...
  return <JourneyFrame><main><StepBanner currentStep={stepNumber} /><div className={styles.lessonLayout}><LessonSidebar stepNumber={stepNumber} lessons={lessons} currentId={parentId} /><article className={styles.lessonContent}><Link href={parent?.href ?? `/journey/step-${stepNumber}`} className={styles.backLink}>← Back to {parentTitle}</Link><header className={styles.lessonHeader}><div><p className={styles.eyebrow}>{sectionId} · {parentId}</p><h1>{title}</h1></div><button className={styles.listenButton} type="button" disabled>◖ Listen to This Study</button></header><section className={styles.audioBlock} aria-label="Listen to this study"><strong>Listen to This Study</strong><p>The approved audio for this study will appear here with the complete readable study.</p></section><div className={styles.manuscript}>{children}</div><div className={styles.completionBar}><button className={styles.completeButton} type="button" disabled title="Progress persistence is implemented in Sequence 8">✓ Mark Study Complete</button><p className={styles.progressNote}>Each study is a completion unit. The parent lesson is complete only after all of its studies are complete.</p></div><ContextNav previous={previousSection} center={{ href: parent?.href ?? `/journey/step-${stepNumber}`, label: `Back to ${parentTitle}` }} next={nextSection} /></article></div></main></JourneyFrame>;
}
