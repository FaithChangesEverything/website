...
type LoginFormProps = { message?: string; next?: string };

export default function LoginForm({ message, next }: LoginFormProps) {
  ...
  function handleSignup(formData: FormData) {
    if (password !== confirmPassword) { setLocalMessage("Passwords do not match."); return; }
    setLocalMessage(""); signup(formData);
  }
  ...
  return (
    <form>
      ...
      <h3 style={{margin:"0 0 24px 0",textAlign:"center",fontFamily:"Georgia, serif",fontSize:"20px",color:"#173A63"}}>{mode === "login" ? "Welcome Back" : "Create Your Account"}</h3>
      <div style={{display:"grid",gridTemplateColumns:"120px 1fr",columnGap:"14px",rowGap:"14px",alignItems:"center"}}>
